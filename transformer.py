...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.src_embeds = nn.Embedding(len(self.src_vocab), embed_dim, 0)
        self.tgt_embeds = nn.Embedding(len(self.tgt_vocab), embed_dim, 0)
        nn.init.normal_(self.src_embeds.weight, std=1/sqrt(embed_dim))
        nn.init.normal_(self.tgt_embeds.weight, std=1/sqrt(embed_dim))
        ...
        self.logits = GlorotLinear(embed_dim, len(self.tgt_vocab))
        # Share embedding and softmax weights
        self.logits.weight = self.tgt_embeds.weight
    ...
    def encode(self, src_tokens, src_mask=None):
        ...
        """
        # Embed and rescale
        x = self.src_embeds(src_tokens) * sqrt(self.embed_dim)
        # Apply dropout
        x = self.embed_drop(x)
        # Word dropout
    ...
    def forward(self, src_tokens, tgt_tokens, src_mask=None):
        ...
        # Embed target tokens
        h = self.tgt_embeds(tgt_tokens) * sqrt(self.embed_dim)
        h = self.embed_drop(h)
        # Word dropout
    ...
    def decode_step(
        ...
        """
        new_states = []
        h = self.tgt_embeds(tgt_token) * sqrt(self.embed_dim)
        h = self.embed_drop(h)
        # Word dropout
